...
def soundex(string):
	...
	string=sub(r'd|t','3',string)
	string=sub(r'l','4',string)
	string=sub(r'm|n','5',string)
...
def ipa(word):#attempts to generate the IPA transcription of a word based on Zompist SC rules
	...
	consonant=['p','b','t','d','g','k','m','n','ñ','f','v','\+','ð','s','z','\$','%','ç','j','r','l','h']#underlined + and $ are edh and %
	#Rule 1
	replacement1=[['ch','ç'],['sh','$'],['ph','f'],['th','+'],['qu','kw'],['wr','r'],['wh','w'],['xh','x'],['rh','r']]
	...
	word=sub('^tm','m',word)
	#Rule 10
	if compile('^[^aeiou]{0,3}y$').fullmatch(word)!=None:
		word=sub('y','ï',word)
	#Rule 11
	word=sub('ey','ë',word)
	word=sub('ay','ä',word)
	word=sub('oy','öy',word)
	#Rule 12
	if compile('[^aeiou]y').fullmatch(word)!=None:
		word=sub('y','i',word)
	#Rule 13
	if compile('stl[aeiouy]$').fullmatch(word)!=None:
	...
	#Rule 24
	if compile('[^aeiou]le$').fullmatch(word)!=None:
		word=sub('le$','@l',word)
	if compile('[^aeiou]re$').fullmatch(word)!=None:
		word=sub('re$','@r',word)
	#Rule 27
	...
	#Rule 33
	if compile('[aeiou].*e$').fullmatch(word)!=None:
		word=sub('e$','',word)
	#Rule 34
	for c in consonant:
		for v in vowel:
	...
	#Rule 35
	if compile('^[^aeiouäëïöüâêîôûòù@]*i[aeiouäëïöüâêîôûòù@]').fullmatch(word)!=None:
		word=list(word)
		spot=word.index('i')
		word[spot]='ï'
		word[spot+1]='@'
...
def frenchipa(word):#https://en.wikipedia.org/wiki/French_orthography
	...
	consonant=['b','c','ç','d','f','g','h','j','k','l','m','n','p','q','ʁ','s','t','v','w','x','z','ŋ','ʒ','ʃ']
	...
	word=sub('ph','f',word)
	#q rules
	if compile('q[^u]').fullmatch(word)!=None:
		word=sub('q','k',word)
	#t rules
	word=sub('t+$','',word)
	word=sub('t+','t',word)
	...
	if compile('o$|oz').fullmatch(word)==None:
		word=sub('o','ɔ',word)
	for v in vowel:
...
def germanipa(word):
	...
	word=sub('d$','t',word)
	#c rules
	word=sub('c(?=ä|e|i)','ts',word)
	word=sub('chs','ks',word)
	word=sub('ck','k',word)
	word=sub('(?<=a|o|u)ch','x',word)
	word=sub('ch','ç',word)
	word=sub('c','k',word)
	#g rules
	word=sub('ig$','iç',word)
