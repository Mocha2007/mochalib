...
class Phonotactics:
	def __init__(self, syllable_structure: Tuple[Tuple[Tuple[Set[Phoneme], bool]]], # bool is for "is this mandatory?"
				constraints: Tuple[Tuple[Set[Phoneme], Environment, bool]], syllable_count: (int, int) = (1, 3)):
		self.syllable_strcuture = syllable_structure
		self.constraints = constraints
		self.syllable_count = syllable_count

	def generate_morpheme(self) -> Tuple[Phoneme]:
		morph = [] # type: List[Phoneme]
		for _ in range(*self.syllable_count):
			...
			# morph.append(tuple(syllable))
		morph = tuple(morph) # type: Tuple[Phoneme]
		return morph if self.obeys(morph) else self.generate_morpheme()

	def obeys(self, word: Tuple[Phoneme]) -> bool:
		for i, phoneme in enumerate(word):
			for phoneme_set, environment, b in self.constraints:
				if phoneme not in phoneme_set:
					continue
				before = word[i-1] if i-1 else None
				after = word[i+1] if i+1 < len(word) else None
...
syllable_a = ({phoneme_b}, False), ({phoneme_a}, True)
